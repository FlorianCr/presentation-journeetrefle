...
::: {.notes}
Maintenant que j'ai expliqué le principe, quelques exemples d'appli
:::
...
::: {.notes}
Tomo: Principe recons 3D à partir d'img 2D

obj: Diminuer le nb d'angles

retombe sur pb similaire à l'illus prec

on divise par 2 le nb points du signal bleu et reconstruit vert

on arrive à recons l'allure mais signal très bruité. Résults prélim, Jordan a amélio la quali du signal depuis
:::
...
::: {.notes}
Maintenant / méthode pt à pt

Principe FS: On envoie une impuls laser IR, on regarde la réponse et on en déduit des prop thermiques (ici l'effusiv)

Pour avoir 1 img, à répéter pr chque pt voir sur la vid chque pt prend 3s, x32^2 d'où l'intéret de dim nb de mesures

sous ech aléatoire de 100% (sert de réf ici) on passe à 20 et on réussit à reconstruire l'img avec la struct de l'ech tjrs visible, mais qqes pertes de détails
:::
